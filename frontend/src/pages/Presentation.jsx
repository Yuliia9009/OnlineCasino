...
function Dot({ active, onClick }) {
    return (
        <button
            onClick={onClick}
            className={`w-2.5 h-2.5 rounded-full transition-all ${active ? "bg-emerald-400 ring-2 ring-emerald-400/40 scale-110" : "bg-gray-600 hover:bg-gray-500"
                }`}
        ...
        >
            ...
            <div className="relative overflow-hidden rounded-xl border border-gray-800/60 bg-gray-900/60 backdrop-blur-md p-6 md:p-10 shadow-[0_0_40px_-15px_rgba(16,185,129,0.4)] h-[540px] md:h-[600px] lg:h-[640px]">
                ...
                {current.type === "title" ? (
                        ...
                        {Array.isArray(current.subtitleLines) && (
                            <div className="mt-5 space-y-1">
                                {current.subtitleLines.map((line, i) => (
                                    <p key={i} className="text-gray-300 md:text-lg">{line}</p>
                                ))}
                            </div>
                        )}
                        {current.footnote && (
                    ...
                    <div className={`grid ${hasImage ? 'md:grid-cols-2' : 'grid-cols-1'} gap-10 items-stretch h-full`}>
                        {/* Текст */}
                        <div className={`space-y-5 overflow-y-auto ${hasImage ? 'pr-2' : ''}`}>
                            <h2 className="text-3xl font-semibold text-white flex items-center gap-3">
                                {current.emoji && <span className="emoji text-2xl leading-none align-middle">{current.emoji}</span>}
                                <span
                                    className="bg-gradient-to-r from-emerald-300 via-teal-200 to-blue-300 bg-clip-text text-transparent drop-shadow-[0_0_12px_rgba(16,185,129,0.2)]"
                                >
                                    {current.title}
                                </span>
                            </h2>
                            ...
                            {current.bullets && (
                                <ul className="space-y-3">
                                    {current.bullets.map((b, i) => (
                                        <li key={i} className="text-gray-200/95 leading-relaxed flex gap-2">
                            ...
                            {current.closing && (
                                <div className="pt-2 text-emerald-300/90 font-medium">{current.closing}</div>
                            )}
                        </div>
            ...
            {/* Zoom Modal */}
            <div
                className={`fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 transition-opacity duration-300 ${zoomed ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
                    }`}
